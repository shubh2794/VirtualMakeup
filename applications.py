...
class makeup(object):
    ...
    def apply_eyeshadow(self, landmarks_array, R, G, B):
        ...
        self.y = list(map(lambda x: int(x), self.y))
        ...
        cv2.fillConvexPoly(filter,np.array(c_[self.yleft[:len(self.xleft)], self.xleft], dtype = 'int32'),1)
        cv2.fillConvexPoly(filter,np.array(c_[self.yright[:len(self.xright)], self.xright], dtype = 'int32'),1)
